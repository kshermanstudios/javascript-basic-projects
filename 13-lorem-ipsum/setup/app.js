...
form.addEventListener('submit', function(e){
  ...
  // force this value to be a number
  const value = parseInt(amount.value);

  // const random = Math.floor(Math.random() * 2);
  const random = Math.floor(Math.random() * text.length); // 9

  console.log('text.length ' + text.length);
  console.log('random ' + random);
  console.log('type ' + typeof random);

  // shows as blue so its a numb otherwise it's black
  console.log(value);
  ...
  if(isNaN(value) || value <= 0 || value > 9){
    // display 1 random paragraph
    result.innerHTML = `<p class="result">${text[random]}</p>`;
  } else {
    let tempText = text.slice(0, value);
    console.log(tempText);

    // simple version - my solution
    // tempText = tempText.join('</p><p class="result">');

    // this is the instrctors version
    // which 
    tempText = tempText.map(function(item){
      return `<p class="result">${item}</p>`;
    }).join(''); // this join clears the ',' between 
